...
from ui.overlays import (draw_overlay, draw_inventory, draw_merchant,
                         draw_dialog)
from ui.journal_ui import draw_journal, CATEGORIES

from .floating_text import FloatingText
from .helpers import ITEM_SYMBOL, merchant_blocked, merchant_close_gate, merchant_open_gate
...
class Game:
    def __init__(self):
        ...
        self._palace_warning_active: bool = False
    ...
    def log(self, msg: str):
        ...
        # Scrivi anche sul diario se disponibile
        if getattr(self, "journal", None) and getattr(self, "player", None):
            age = getattr(self.player, "age", 0)
            try:
                self.journal.add(self.day_number, age, msg, "misc")
            except Exception:
                pass

    def _show_palace_warning(self, guard=None):
        """Mostra l'avviso del palazzo come dialogo, non solo come log."""
        palace_guard = guard
        if palace_guard is None:
            palace = next((b for b in self.world.buildings if getattr(b, "btype", "") == "palace"), None)
            palace_guards = [e for e in self.entities if getattr(e, "is_palace_guard", False) and e.alive]
            if palace and palace_guards and getattr(self, "player", None):
                p = self.player
                palace_guard = min(palace_guards, key=lambda g: abs(g.x - p.x) + abs(g.y - p.y))
        if palace_guard is None:
            return
        self.dialog_ent = palace_guard
        self.dialog_text_override = "Non avvicinarti al re o alla regina!"
        self.state = GameState.DIALOG
        self._palace_warning_active = True
        palace_guard.warned_once = True
        palace_guard.guard_stage = max(getattr(palace_guard, "guard_stage", 0), 1)

    def _palace_entry_zone(self, palace, x: int, y: int) -> bool:
        """Tile esatto subito oltre la porta del palazzo."""
        if not palace:
            return False
        return x == palace.door_x and y == palace.door_y - 1

    def _palace_door_tile(self, palace):
        if not palace:
            return None
        return (palace.door_x, palace.door_y)

    def _palace_inner_tile(self, palace):
        if not palace:
            return None
        return (palace.door_x, palace.door_y - 1)
    ...
    def start_new_game(self):
        ...
        self.spawner     = WorldSpawner(self.world)
        self.settlements = WorldSettlements(self.world)
        self._palace_warning_active = False
    ...
    def save_game(self):
        try:
            data = {
                ...
                "journal":         self.journal.to_dict() if self.journal else {},
                "settlements": {
    ...
    def try_move(self, dx: int, dy: int):
        ...
        palace = next((b for b in self.world.buildings if getattr(b, "btype", "") == "palace"), None)
        palace_entry_step = False
        palace_inside_after_move = False
        if palace:
            palace_entry_step = (
                (ox, oy) == self._palace_door_tile(palace)
                and (nx, ny) == self._palace_inner_tile(palace)
            )
            palace_inside_after_move = palace.wx <= nx < palace.wx + palace.w and palace.wy <= ny < palace.wy + palace.h
        for e in self.entities:
            if e.alive and e.x == nx and e.y == ny:
                if getattr(e, "is_palace_guard", False):
                    self._show_palace_warning(e)
                    return
                ...
                return
        p.x, p.y = nx, ny
        if palace and palace_entry_step:
            if not getattr(self, "_palace_warning_active", False):
                palace_guards = [e for e in self.entities if getattr(e, "is_palace_guard", False) and e.alive]
                if palace_guards:
                    nearest = min(palace_guards, key=lambda g: abs(g.x - p.x) + abs(g.y - p.y))
                    self._show_palace_warning(nearest)
        elif palace and not palace_inside_after_move:
            palace_guards = [e for e in self.entities if getattr(e, "is_palace_guard", False) and e.alive]
            for g in palace_guards:
                g.warned_once = False
                g.guard_stage = 0
            self._palace_warning_active = False
        ...
        for item in list(self.items_on_ground):
            if item.x == p.x and item.y == p.y:
                ...
                if getattr(item, "rarity", "common") in ("rare", "epic", "legendary"):
                    self.journal_add(f"Raccolto {item.name} ({item.rarity})", "item")
            ...
            if abs(e.x-p.x) <= _interact_r and abs(e.y-p.y) <= _interact_r:
                ...
                if getattr(e, "npc_role", "") == "ins_magia":
                    ...
                    if not getattr(p, "magic_revealed", False):
                        ...
                        self.state = GameState.DIALOG
                        return
                if e.ai_type == "royal" or getattr(e, "is_royal", False):
                    self.dialog_ent = e
                    self.dialog_text_override = "Guardie! Fermatelo!"
                    self.state = GameState.DIALOG
                    return
                if getattr(e, "is_palace_guard", False):
                    self._show_palace_warning(e)
    ...
    def handle_death(self):
        ...
        self.journal_add(f"Morte a {p.age} anni: {p.death_cause}", "death")
        try:
    ...
    def _end_combat(self):
        ...
        for lv in p.add_xp(cs.xp_gained):
            ...
            self.journal_add(f"Nuovo livello {lv}", "level")
    ...
    def _update_merchant_ai(self, e, passable):
        ...
        if tod == "NIGHT":
            ...
            if (e.x, e.y) == (retro_x, retro_y):
                merchant_close_gate(bottega, world)
            else:
                merchant_open_gate(bottega, world)
                ...
                if path:
                    nx2, ny2 = path[0]
                    if not merchant_blocked(nx2, ny2, self.entities, e):
                        e.x, e.y = nx2, ny2
            return
        # DUSK
        if tod == "DUSK":
            e.is_outside = False; merchant_close_gate(bottega, world)
            if (e.x, e.y) == (back_x, back_y):
                ...
                if path:
                    nx2, ny2 = path[0]
                    if not merchant_blocked(nx2, ny2, self.entities, e): e.x, e.y = nx2, ny2
            return
        # DAY
        ...
        if getattr(e, "is_outside", False):
            ...
            if dist_dest <= 2:
                ...
                if abs(e.x-back_x)+abs(e.y-back_y) <= 1:
                    ...
                    merchant_close_gate(bottega, world); e.x, e.y = spawn_x, spawn_y
            else:
            ...
            if (e.x, e.y) == (back_x, back_y):
                ...
                e.is_outside = True; merchant_close_gate(bottega, world)
        else:
            if _r.random() < 0.25 and zone:
                cands = list(zone); _r.shuffle(cands)
                for zx, zy in cands:
                    if not merchant_blocked(zx, zy, self.entities, e):
                        e.x, e.y = zx, zy; break
    ...
    def _update_innkeeper_ai(self, e, passable):
        ...
        if tod == "NIGHT":
            if (e.x, e.y) == (retro_x, retro_y):
                merchant_close_gate(locanda, world)
            else:
                merchant_open_gate(locanda, world)
    ...
    def update_entities(self, dt: float):
        ...
        for e in list(self.entities):
            ...
            if e.ai_type == "aggressive":
                ...
                for other in self.entities:
                    if other is e or not other.alive: continue
                    if other.ai_type in ("wander", "flee", "npc", "merchant", "innkeeper"):
                ...
                if dist <= int(6*nm):
                    if dist == 1:
                        ...
                        if path: e.x,e.y=path[0]
                            ...
                            e.x+=dx_; e.y+=dy_
            elif e.ai_type in ("merchant", "innkeeper", "npc", "wander"):
                if e.ai_type == "merchant" and getattr(e, "npc_role", "") == "merchant":
                    ...
                    engine = getattr(self, "npc_engine", None)
                    if engine is None:
                        engine = NPCBehaviorEngine()
                        self.npc_engine = engine
                    engine.update(e, self.world, self.entities, passable, p, self.get_game_hour(), self.get_time_of_day(), dt)
                    continue
            elif e.ai_type == "royal":
                pass
            elif e.ai_type == "guard":
                px_ = getattr(e, "patrol_x", getattr(e, "home_x", e.x))
                py_ = getattr(e, "patrol_y", getattr(e, "home_y", e.y))
                is_palace_guard = getattr(e, "is_palace_guard", False)
                guard_stage = getattr(e, "guard_stage", 0)

                palace = next((b for b in self.world.buildings if getattr(b, "btype", "") == "palace"), None)
                if is_palace_guard and palace:
                    inside_palace = palace.wx <= p.x < palace.wx + palace.w and palace.wy <= p.y < palace.wy + palace.h
                    royal_positions = [
                        (other.x, other.y)
                        for other in self.entities
                        if getattr(other, "is_royal", False) and other.alive
                    ]
                    ...
                    if royal_positions and guard_stage < 2:
                        nearest_royal = min(
                            royal_positions,
                            key=lambda pos: abs(pos[0] - p.x) + abs(pos[1] - p.y),
                        )
                        if abs(nearest_royal[0] - p.x) + abs(nearest_royal[1] - p.y) <= 6:
                            e.guard_stage = 2
                            guard_stage = 2
                            self.log("Guardia Reale: \"Non avvicinarti al re o alla regina!\"")
                ...
                # Cerca nemici aggressivi nel raggio 12
                target = None
                for other in self.entities:
                ...
                if target:
                    ...
                    if td == 1:
                        ...
                        self.floating_texts.append(FloatingText(str(dmg), sx, sy, (255, 200, 50), 1.2))
                        if target.health <= 0:
                            target.alive = False
                    else:
                        path = astar(e.x, e.y, target.x, target.y, passable, 20)
    ...
    def render(self):
        ...
        if s == GameState.SPLASH:
            ...
            if self.player:
                ...
                elif s == GameState.JOURNAL and self.journal:
                    draw_journal(self.screen, self.fonts, self.journal,
                                 self.journal_cursor, self.journal_tab)
    ...
    def handle_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.type == pygame.KEYDOWN and self.journal:
                    k = event.key
                    ...
                    if k in (pygame.K_ESCAPE, pygame.K_j) or uni == "j":
                        ...
                        # clamp using current category size
                        cat = CATEGORIES[self.journal_tab]
                        entries = (self.journal.entries if cat == "all"
                                   else self.journal.get_by_category(cat))
